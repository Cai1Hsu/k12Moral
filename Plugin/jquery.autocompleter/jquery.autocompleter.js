 ...
 * Edit By tony.jiang 2018-04-01
 * 1、修复自定义参数配置的bug
 * 2、增加"delay"参数并实现
 * 3、增加 changecallback  方法，实现数据变动时进行回调（当清空数据/数据变动有特殊操作时可能需要用到）
 */
...
; (function ($, window) {
     ...
     */
    function _search(query, source, limit, customLabel) {
        var response = [];
            ...
            for (var i = 0; i < 2; i++) {
                for (var item in source) {
                    if (response.length < limit) {
                        var label = (customLabel && list[item][customLabel]) ? list[item][customLabel] : list[item].label;
                        switch (i) {
                            case 0:
    ...
    function _launch(data) {
        ...
        _launchTimeout = setTimeout(function () { _launchdelay(data); }, delay)
        //_launchdelay(data);
    }
    ...
    * @method private
    * @name _launchdelay
    * @description Use source locally or create xhr with delay
    * @param data [object] "Instance data"
    */
    function _launchdelay(data) {
        data.query = $.trim(data.$node.val());
        data.$node.removeAttr('data-value');
        data.changecallback.call(data.$autocompleter, data.query);
        if (!data.empty && data.query.length === 0) {
            ...
            if (typeof data.source === 'object') {
                ...
                // Local search
                var search = _search(data.query, _clone(data.source), data.limit, data.customLabel);
                if (search.length) {
    ...
    function _buildList(list, data) {
        ...
        for (var item = 0, count = list.length; item < count; item++) {
            ...
            if (data.template) {
                var lablefeild = data.customLabel ? data.customLabel : 'label';
                var template = data.template.replace(/({{ " + lablefeild + " }})/gi, label);
                for (var property in list[item]) {
                    if (list[item].hasOwnProperty(property)) {
                        var regex = new RegExp('{{ ' + property + ' }}', 'gi');
                        var content;
                        if (data.customLabel && data.customLabel == property) {
                            content = label
                        }
                        else {
                            content = list[item][property];
                        }
        ...
        if (list.length && data.hint) {
            ...
            var firstlabel = list[0][data.customLabel];
            var hint = (firstlabel.substr(0, data.query.length).toUpperCase() === data.query.toUpperCase()) ? firstlabel : false;
            if (hint && (data.query !== list[0].CName)) {
    ...
    function _setValue(data) {
        if (data.$selected) {
            ...
            var lable = data.$selected.attr('data-label');
            data.$node.attr("data-value", value);
            data.$node.val(lable);
        } else {
            if (data.hintText && !data.$autocompleter.find('.autocompleter-hint').hasClass('autocompleter-hint-show')) {
    ...
    function _handleChange(data) {
        data.callback.call(data.$autocompleter, data.$node.val(), data.$node.attr('data-value'), data.index, data.response[data.index]);
